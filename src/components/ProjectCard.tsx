...
const ProjectCard = ({ id, handleCardClick, item }: ProjectCardProps) => {
    return (
        <Link href={"#"} onClick={(e) => {
            e.preventDefault();
            handleCardClick(id);
        }} className={`p-4 pb-8 rounded-2xl bg-[${item.color}] min-h-36 flex flex-col justify-center items-start`}>
            <div className="max-h-80 rounded-2xl overflow-hidden p-3.5 bg-white flex items-center justify-center">
                <svg width="80" height="80" viewBox="0 0 800 800" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path d="M667.734 646.953H132.734C102.344 646.953 77.6562 622.266 77.6562 591.875V202.031C77.6562 171.641 102.344 146.953 132.734 146.953H667.656C698.047 146.953 722.734 171.641 722.734 202.031V591.875C722.812 622.266 698.125 646.953 667.734 646.953Z" fill="white" />
                    ...
                    <path d="M166.641 588.125H400.078C404.375 588.125 407.891 584.609 407.891 580.313C407.891 576.016 404.375 572.5 400.078 572.5H166.641C160 572.5 154.609 567.109 154.609 560.469V409.844C154.609 405.547 151.094 402.031 146.797 402.031C142.5 402.031 138.984 405.547 138.984 409.844V560.547C139.063 575.703 151.406 588.125 166.641 588.125Z" fill={item.hexa} />
                    ...
                    <path d="M668.125 146.953H132.344C102.187 146.953 77.6562 171.484 77.6562 201.641V592.266C77.6562 622.422 102.187 646.953 132.344 646.953H668.047C698.203 646.953 722.734 622.422 722.734 592.266V201.641C722.812 171.484 698.281 146.953 668.125 146.953ZM691.562 592.266C691.562 605.156 681.016 615.703 668.125 615.703H132.344C119.453 615.703 108.906 605.156 108.906 592.266V311.094H691.484V592.266H691.562ZM691.562 295.469H108.984V201.641C108.984 188.75 119.531 178.203 132.422 178.203H668.125C681.016 178.203 691.562 188.75 691.562 201.641V295.469Z" fill={item.hexa} />
                    <path d="M359.297 487.969L270 452.578L359.297 418.437V382.109L231.328 437.5V468.359L359.297 523.984V487.969ZM444.297 355.312H417.422L371.172 548.438H397.578L444.297 355.312ZM584.297 437.734L456.172 382.344V418.125L545.547 452.891L456.172 488.047V524.141L584.297 468.359V437.734Z" fill={item.hexa} />
                </svg>

            </div>
            <pre className="py-4 text-sm leading-6">
                <span className={item.textColor}><span className={item.varColor}>const</span> projectType ={" "}</span>
                <span className={item.contentColor}>"{item.projectType};"</span>
                {"\n"}
                <span className={item.textColor}><span className={item.varColor}>const</span> techStack ={" "}</span>
                <span className={item.contentColor}>{"() =>"} ('{item.techStack}');</span>
                {"\n"}
                <span className={item.textColor}>
                    <span className={item.contentColor}>
                        <span className={item.textColor}>
                            <span className={item.varColor}>
                                function () {" "}
                            </span>
                            {"{"}
                        </span>
                        {"\n"}  return{"("} console.log("Click To Open Model") {")"};
                        {"\n"}
                        {"}"}
                    </span>
                </span>
